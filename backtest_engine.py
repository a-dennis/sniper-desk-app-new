...
def run_symbol_backtest(yahoo_ticker: str, display_symbol: str = None, lookahead_minutes: int = 15,
                         min_score: int = 4) -> dict:
    """Backtests one symbol over the last ~7 days of real 1-minute data.
    `yahoo_ticker` is the actual Yahoo Finance symbol (e.g. "RELIANCE.NS",
    "GC=F", "BTC-USD"); `display_symbol` is what's shown in messages
    (defaults to yahoo_ticker if not given).
    ...
    Returns a dict with a "verdict" of PASS, FAIL, or INSUFFICIENT_DATA:
      ...
      - INSUFFICIENT_DATA: fewer than 10 qualifying historical bars found
        ...
        traded symbols or ones that rarely hit the score bar may not have
        enough history to say anything reliable).
    """
    symbol = display_symbol or yahoo_ticker
    try:
        df = yf.Ticker(yahoo_ticker).history(period="7d", interval="1m")
