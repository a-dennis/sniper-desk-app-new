...
def fresh_vwap_cross_check(df: pd.DataFrame) -> tuple[bool, str]:
    """Detects a stock that JUST crossed above VWAP in the most recent bar
    (not one that's been sitting above VWAP for a while) combined with a
    volume surge on that same bar -- catches the moment of a move starting,
    rather than a move that's already underway or exhausted.
    """
    if len(df) < 22:
        return False, "Need 21+ 1-min bars to check for a fresh cross"
    typical = (df["high"] + df["low"] + df["close"]) / 3
    cum_vol = df["volume"].cumsum()
    vwap_series = (typical * df["volume"]).cumsum() / cum_vol.replace(0, 1)

    prev_close, prev_vwap = df["close"].iloc[-2], vwap_series.iloc[-2]
    curr_close, curr_vwap = df["close"].iloc[-1], vwap_series.iloc[-1]
    crossed_up = prev_close <= prev_vwap and curr_close > curr_vwap
    if not crossed_up:
        return False, f"No fresh cross this bar (price {'above' if curr_close > curr_vwap else 'below'} VWAP already, or still below)"

    last_vol = df["volume"].iloc[-1]
    avg_prev20 = df["volume"].iloc[-21:-1].mean()
    ratio = (last_vol / avg_prev20) if avg_prev20 > 0 else 0
    if ratio >= 2.0:
        return True, f"Just crossed above VWAP ({curr_vwap:.2f}) this bar with {ratio:.2f}x volume surge"
    return False, f"Crossed above VWAP this bar, but volume only {ratio:.2f}x (needs 2.0x+)"
