...
if evaluations:
    ...
    # ---- Backtest-Validated Pick: among today's Top 5 qualifiers, find one
    # that ALSO has a history of working out, not just a live signal ----
    qualifiers = [e for e in top5 if e["technical_score"] >= MIN_WINNING_SCORE]
    if qualifiers:
        with st.spinner("Cross-checking Top 5 against 7-day historical performance..."):
            bt_candidates = []
            for e in qualifiers:
        ...
        if bt_candidates:
            ...
            if len(bt_candidates) > 1:
                st.caption(f"{len(bt_candidates)} of today's Top 5 passed backtest validation -- showing the strongest by historical win rate.")
        else:
            st.info("None of today's Top 5 qualifying stocks have passed backtest validation yet (either the historical win rate was 50% or below, or there wasn't enough 7-day history to judge). The live 4/5+ signal above still stands on its own -- this second check is an extra filter, not a requirement.")
    else:
