...
if evaluations:
    ...
    if st.button("🔍 Check Backtest-Validated Picks", disabled=not qualifiers):
        with st.spinner(f"Cross-checking {len(qualifiers)} qualifying stocks against 7-day historical performance..."):
            ...
            st.session_state.bt_candidates = bt_candidates[:3]
    ...
    elif "bt_candidates" in st.session_state:
        bt_candidates = st.session_state.bt_candidates
        if bt_candidates:
            ...
            st.markdown(compact_card(
                "✅ BACKTEST PASS " + str(bidx + 1) + " OF " + str(len(bt_candidates)), "#ffffff", "#0d9488",
                best_e["symbol"], format(best_e["ltp"], ",.2f"),
                "#0d9488", "linear-gradient(135deg, #ccfbf1 0%, #99f6e4 100%)",
                "LIVE SCORE", str(best_e["technical_score"]) + "/5", "#15803d",
                "WIN RATE", str(best_r["win_rate"]) + "%", "#0d9488",
                "AVG MOVE", format(best_r["avg_return"], "+.2f") + "%", "#0d9488" if best_r["avg_return"] >= 0 else "#b91c1c",
            ), unsafe_allow_html=True)
            ...
            st.info("None of the qualifying stocks passed backtest validation (either the historical win rate was 50% or below, or there wasn't enough 7-day history to judge). The live 4/5+ signal above still stands on its own -- this is an extra filter, not a requirement.")
