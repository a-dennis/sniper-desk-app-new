...
from trade_journal import render_trade_journal, load_journal
from sector_data import get_sector
from ai_insight import get_ai_insight, get_market_pulse, get_journal_coaching
...
with st.sidebar:
    ...
    NAV_PAGES = ["Dashboard", "Winner Scanner", "Options Strategy Ideas", "Market Heatmap", "Sector Analysis",
...
def render_rating_gauge(score, max_score=5):
    ...
    <div style="text-align:center;font-size:1.4em;font-weight:700;color:{color};margin-top:4px;">{label}</div>
    """
    st.markdown(svg, unsafe_allow_html=True)


def render_signals_table(rows, timing_rows=None, context_rows=None, ai_row=None):
    """One consistent, disciplined table for every test we run on a stock --
    grouped by category (TECHNICAL / TIMING / CONTEXT / AI), same visual
    treatment for every row. Replaces the old scattered mix of banners,
    bullet lists, and floating cards that were overlapping each other.

    `rows`: the snapshot's own (name, verdict, detail) tuples -- verdict is
        "pass"/"fail"/"unavailable".
    `timing_rows`, `context_rows`: each a list of (name, label, color, detail)
        tuples for rows that don't fit the pass/fail/unavailable shape (e.g.
        "WATCHING" in orange, "DOWNTREND" in red).
    `ai_row`: optional (label, color, detail) tuple for the AI Insight row.
    """
    verdict_style = {
        "pass": ("PASS", "#0a7d2e"), "fail": ("FAIL", "#c81e1e"), "unavailable": ("N/A", "#6b7280"),
    }

    def row_html(icon, name, label, color, detail):
        return (
            "<tr><td style='width:28px;text-align:center;'>" + icon + "</td>"
            "<td><b>" + name + "</b></td>"
            "<td><span style='background:" + color + ";color:#fff;padding:2px 10px;border-radius:4px;"
            "font-size:0.8em;font-weight:700;'>" + label + "</span></td>"
            "<td style='color:#4b5563;font-size:0.9em;'>" + detail + "</td></tr>"
        )

    def section_header(title):
        return ("<tr><td colspan='4' style='background:#f1f5f9;font-weight:700;font-size:0.78em;"
                "color:#475569;text-transform:uppercase;letter-spacing:0.04em;padding:6px 10px;'>" + title + "</td></tr>")

    body = section_header("Technical")
    for name, verdict, detail in rows:
        label, color = verdict_style.get(verdict, verdict_style["unavailable"])
        icon = {"pass": "✅", "fail": "❌", "unavailable": "⚪"}[verdict]
        body += row_html(icon, name, label, color, detail)

    if timing_rows:
        body += section_header("Timing")
        for name, label, color, detail in timing_rows:
            body += row_html("🕐", name, label, color, detail)

    if context_rows:
        body += section_header("Context")
        for name, label, color, detail in context_rows:
            body += row_html("📊", name, label, color, detail)

    if ai_row:
        body += section_header("AI")
        label, color, detail = ai_row
        body += row_html("✨", "Gemini Insight", label, color, detail)

    table_html = ('<table class="qb-text" style="width:100%; border-collapse:collapse; background:#ffffff; '
                  'border:1px solid #d1d5db; border-radius:8px;"><tbody>' + body + '</tbody></table>')
    st.markdown(table_html, unsafe_allow_html=True)


# ============================================================ OPTIONS IDEAS
def get_directional_bias(rows):
    """Educational directional read from our own real signals (VWAP, EMA
    cross, Supertrend, Momentum -- skipping Volume Surge since that's
    magnitude, not direction). Returns (bias, color, bullish_count,
    bearish_count) out of 4 directional checks. This is NOT options-aware --
    it has no premium/IV/OI data, it's purely the underlying stock's
    technical direction."""
    directional = [r for r in rows if r[0] in ("VWAP Support Anchoring", "EMA 9 / 21 Cross",
                                                 "Supertrend Speed Engine", "Intraday Momentum Acceleration")]
    bullish = sum(1 for _, v, _ in directional if v == "pass")
    bearish = sum(1 for _, v, _ in directional if v == "fail")
    if bullish >= 3:
        return "BULLISH", "#166534", bullish, bearish
    if bearish >= 3:
        return "BEARISH", "#c81e1e", bullish, bearish
    return "NEUTRAL", "#6b7280", bullish, bearish


def suggest_strategy_type(bias, bullish_count, bearish_count, regime):
    """Purely educational strategy-TYPE suggestion -- no premium, strike, or
    expiry data involved (we don't have real options data). Deliberately
    favors defined-risk spread structures over naked long options, per the
    documented pattern that far-OTM naked buying drives most retail F&O
    losses (see SEBI study discussion). This is not a trade recommendation."""
    if bias == "BULLISH":
        if bullish_count == 4 and regime != "CHOPPY/NEUTRAL":
            return "Bull Call Spread (or Long Call if you accept faster time decay)", \
                   "Strong, consistent bullish signal across all 4 directional checks."
        return "Bull Call Spread preferred over a naked Call", \
               "Bullish but not unanimous -- a spread reduces cost and time-decay risk versus buying a call outright."
    if bias == "BEARISH":
        if bearish_count == 4 and regime != "CHOPPY/NEUTRAL":
            return "Bear Put Spread (or Long Put if you accept faster time decay)", \
                   "Strong, consistent bearish signal across all 4 directional checks."
        return "Bear Put Spread preferred over a naked Put", \
               "Bearish but not unanimous -- a spread reduces cost and time-decay risk versus buying a put outright."
    if regime == "CHOPPY/NEUTRAL":
        return "Iron Condor / range-bound spread (if comfortable with premium-selling)", \
               "Signals are mixed and the broader market regime is choppy -- a defined-risk range strategy fits better than a directional bet."
    return "No clear directional edge -- consider staying out", \
...
def render_options_ideas_page(segment, evaluations):
    ...
    for e in evaluations[:15]:
        ...
        rows_html += (
            ...
            "<td><b>" + strategy + "</b></td>"
            "<td style='color:#4b5563;font-size:0.85em;'>" + rationale + "</td></tr>"
        )
    table_html = ('<table class="qb-text" style="width:100%; border-collapse:collapse; background:#ffffff; '
...
def render_dashboard(segment, evaluations, top10, shortlist):
    ...
    # ---- Daily AI Market Pulse ----
    with st.container():
        st.markdown(
            '<div style="background:linear-gradient(135deg, #f5f3ff 0%, #ede9fe 100%);border:2px solid #7c3aed;'
            'border-radius:14px;padding:14px 18px;margin-bottom:14px;">'
            '<div style="font-weight:700;color:#5b21b6;margin-bottom:6px;">✨ Daily AI Market Pulse (Gemini)</div>',
            unsafe_allow_html=True,
        )
        if st.button("🤖 Generate Today's Market Pulse", key="market_pulse_btn"):
            with st.spinner("Asking Gemini to summarize today's scan..."):
                regime_now = get_market_regime()["regime"] if segment in EQUITY_LIKE_SEGMENTS else "N/A"
                pulse_news = fetch_news("Nifty Sensex market news India" if segment in EQUITY_LIKE_SEGMENTS else segment + " market news")
                pulse_result = get_market_pulse(segment, regime_now, top10, pulse_news)
            st.session_state["market_pulse_result"] = pulse_result
        pulse_cached = st.session_state.get("market_pulse_result")
        if pulse_cached:
            if pulse_cached["success"]:
                st.markdown('<div style="color:#1e1b2e;">' + pulse_cached["text"] + '</div>', unsafe_allow_html=True)
                st.caption("AI commentary only — not financial advice.")
            else:
                st.warning(f"⚠️ Unavailable: {pulse_cached['error']}")
        else:
            st.caption("Click to get an AI-generated overview of today's scan results, market regime, and news — on demand.")
    ...
    with col_right:
        ...
        if best:
            ...
            render_signals_table(best["rows"][-5:])  # the 5 real technical checks
...
def render_other_page(page, segment, evaluations, top10, shortlist):
    ...
    if page == "Dashboard":
    ...
    elif page == "Options Strategy Ideas":
        render_options_ideas_page(segment, evaluations)
        ...
        st.markdown(
            ...
            '<div style="font-weight:700;color:#5b21b6;margin-bottom:6px;">✨ AI Trade Journal Coach (Gemini)</div>',
            unsafe_allow_html=True,
        )
        journal_df = load_journal()
        if journal_df.empty:
            st.caption("Log a few trades above first -- this analyzes your own real trade history for patterns.")
        else:
            if st.button("🤖 Analyze My Trade History"):
                with st.spinner("Asking Gemini to find real patterns in your logged trades..."):
                    coach_result = get_journal_coaching(journal_df.to_csv(index=False))
                st.session_state["journal_coach_result"] = coach_result
            coach_cached = st.session_state.get("journal_coach_result")
            if coach_cached:
                if coach_cached["success"]:
                    st.markdown('<div style="color:#1e1b2e;">' + coach_cached["text"] + '</div>', unsafe_allow_html=True)
                    st.caption("AI reflection on your own data only — not trading advice for future trades.")
                else:
                    st.warning(f"⚠️ Unavailable: {coach_cached['error']}")
        st.markdown('</div>', unsafe_allow_html=True)
...
# ---------------------------------------------------------- unified table --
_timing_rows = []
if snapshot.get("fresh_vwap_cross"):
    _timing_rows.append(("Fresh VWAP Cross", "YES", "#7c3aed", snapshot.get("fresh_vwap_cross_detail", "")))
_setup_for_table = None
if segment in EQUITY_LIKE_SEGMENTS:
    _setup_for_table = detect_setup(get_intraday_candles(snapshot["yahoo_ticker"], interval="5m"))
    _state_colors = {"READY_TO_ENTER": "#166534", "WATCHING_RETEST": "#b45309",
                      "TOO_LATE_CHASE": "#c81e1e", "NO_SETUP": "#6b7280"}
    _state_labels = {"READY_TO_ENTER": "READY", "WATCHING_RETEST": "WATCHING",
                      "TOO_LATE_CHASE": "TOO LATE", "NO_SETUP": "NO SETUP"}
    _timing_rows.append(("Breakout Setup", _state_labels.get(_setup_for_table["state"], "N/A"),
                          _state_colors.get(_setup_for_table["state"], "#6b7280"), _setup_for_table["detail"]))

_context_rows = []
if segment in EQUITY_LIKE_SEGMENTS:
    _regime_for_table = get_market_regime()
    _regime_colors = {"UPTREND": "#166534", "DOWNTREND": "#c81e1e", "CHOPPY/NEUTRAL": "#b45309"}
    _context_rows.append(("Market Regime", _regime_for_table["regime"],
                           _regime_colors.get(_regime_for_table["regime"], "#6b7280"), _regime_for_table["detail"]))
_bt_for_table = st.session_state.backtest_results.get(snapshot["symbol"])
if _bt_for_table and _bt_for_table["verdict"] in ("PASS", "FAIL"):
    _context_rows.append(("7-Day Backtest", _bt_for_table["verdict"],
                           "#166534" if _bt_for_table["verdict"] == "PASS" else "#c81e1e", _bt_for_table["detail"]))

_ai_cached_for_table = st.session_state.get("ai_insight_cache", {}).get(snapshot["symbol"])
_ai_row_for_table = None
if _ai_cached_for_table and _ai_cached_for_table["success"]:
    _ai_row_for_table = ("Analyzed", "#7c3aed", _ai_cached_for_table["sections"].get("OVERALL TAKE", "See AI Insight panel below for full analysis."))

render_signals_table(snapshot["rows"][-5:], timing_rows=_timing_rows, context_rows=_context_rows, ai_row=_ai_row_for_table)
...
st.markdown(
    ...
    .qb-ai-header { display:flex; align-items:center; gap:8px; font-weight:700; font-size:1.05em; color:#5b21b6; margin-bottom:10px; }
    .qb-ai-section { margin-bottom: 8px; }
    .qb-ai-label { font-size:0.72em; font-weight:700; color:#7c3aed; text-transform:uppercase; letter-spacing:0.04em; }
    .qb-ai-text { color:#1e1b2e; margin-top:2px; }
    .qb-confidence-high { background:#166534; color:#fff; padding:2px 12px; border-radius:12px; font-weight:700; font-size:0.8em; }
    .qb-confidence-moderate { background:#b45309; color:#fff; padding:2px 12px; border-radius:12px; font-weight:700; font-size:0.8em; }
    .qb-confidence-low { background:#991b1b; color:#fff; padding:2px 12px; border-radius:12px; font-weight:700; font-size:0.8em; }
    </style>
    """,
    unsafe_allow_html=True,
)
...
st.markdown('<div class="qb-ai-panel">', unsafe_allow_html=True)
st.markdown('<div class="qb-ai-header">✨ AI Insight (Gemini) — reasoning over our own real data, not a black box</div>', unsafe_allow_html=True)

if st.button("🤖 Get AI Insight for " + snapshot["symbol"], key="ai_insight_btn"):
    with st.spinner("Asking Gemini to reason over the real technical signals + news..."):
        ai_news = fetch_news(snapshot["symbol"] + " " + NEWS_QUERY_SUFFIX[segment])
        ai_breakout_setup = None
        if segment in EQUITY_LIKE_SEGMENTS:
            ai_breakout_setup = detect_setup(get_intraday_candles(snapshot["yahoo_ticker"], interval="5m"))
        ai_backtest_result = st.session_state.backtest_results.get(snapshot["symbol"])
        result = get_ai_insight(snapshot["symbol"], snapshot["ltp"], snapshot["change_pct"],
                                 snapshot["technical_score"], snapshot["rows"], ai_news,
                                 breakout_setup=ai_breakout_setup, backtest_result=ai_backtest_result)
    st.session_state.ai_insight_cache[snapshot["symbol"]] = result

cached_result = st.session_state.ai_insight_cache.get(snapshot["symbol"])
if cached_result:
    if cached_result["success"]:
        sections = cached_result["sections"]
        confidence = sections.get("CONFIDENCE", "").strip().lower()
        conf_class = {"high": "qb-confidence-high", "moderate": "qb-confidence-moderate", "low": "qb-confidence-low"}.get(confidence, "qb-confidence-moderate")

        if "TECHNICAL READ" in sections:
            st.markdown('<div class="qb-ai-section"><div class="qb-ai-label">Technical Read</div>'
                        '<div class="qb-ai-text">' + sections["TECHNICAL READ"] + '</div></div>', unsafe_allow_html=True)
        if "NEWS READ" in sections:
            st.markdown('<div class="qb-ai-section"><div class="qb-ai-label">News Read</div>'
                        '<div class="qb-ai-text">' + sections["NEWS READ"] + '</div></div>', unsafe_allow_html=True)
        if "OVERALL TAKE" in sections:
            st.markdown('<div class="qb-ai-section"><div class="qb-ai-label">Overall Take</div>'
                        '<div class="qb-ai-text">' + sections["OVERALL TAKE"] + '</div></div>', unsafe_allow_html=True)
        if confidence:
            st.markdown('<div class="qb-ai-section"><span class="' + conf_class + '">Confidence: '
                        + sections.get("CONFIDENCE", "") + '</span></div>', unsafe_allow_html=True)
        if not sections:  # fallback if the model didn't follow the exact format
            st.markdown('<div class="qb-ai-text">' + cached_result["text"] + '</div>', unsafe_allow_html=True)
        st.caption("AI commentary only — not financial advice. Based solely on the technical signals and headlines shown elsewhere on this page.")
    else:
        st.warning(f"⚠️ AI Insight unavailable: {cached_result['error']}")
        if "GEMINI_API_KEY" in cached_result["error"]:
            st.caption("Add your free Gemini API key to Streamlit secrets as GEMINI_API_KEY (get one at aistudio.google.com/apikey).")
else:
    st.caption("Click the button above for an AI-generated read combining our real technical signals with the latest news — on demand, to stay within Gemini's free tier.")
st.markdown('</div>', unsafe_allow_html=True)
