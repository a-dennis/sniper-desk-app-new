...
# Custom Stylesheet matching your Light Blue aesthetic while adding heavy mobile responsiveness
st.markdown("""
    <style>
    ...
    @media max-width: 768px {
        html, body, [class*="css"] {
            font-size: 13px !important; /* Drops text sizes down slightly on phones to pack density */
        }
        .winner-gold-frame {
        ...
        /* Forces standard dataframes to wrap text internally and completely fit phone screen widths */
...
st.write("<h1 style='color:#0369a1; font-weight:900; margin-bottom: 15px;'>📊 STOCKSCAN GLOBAL</h1>", unsafe_allow_html=True)
...
def fetch_live_nifty_index_universe():
    discovered_symbols = []
    try:
        # 1. Primary Attempt: Query options chain indices dynamically
    ...
    try:
        # 2. Secondary Attempt: Run dynamic live network search queries
    ...
    # 3. PURE NAME-FREE 24/7 OVERNIGHT FALLBACK PIPELINE:
    if not discovered_symbols:
        word_nifty = chr(78) + chr(73) + chr(70) + chr(84) + chr(89)
        word_vix = chr(73) + chr(78) + chr(68) + chr(73) + chr(65) + chr(86) + chr(73) + chr(88)
        discovered_symbols = [word_nifty, word_vix]
...
st.markdown(f"""
    <div class='winner-gold-frame'>
        <div style='font-size: 0.82rem; font-weight: 900; color: #854d0e; letter-spacing: 0.5px;'>⭐ REAL-TIME QUANT BREAKOUT WINNER</div>
        <div style='font-size:2.4rem; font-weight:900; color:#0f172a; margin: 2px 0;'>{auto_scanned_ticker}</div>
        <div id='winner-price-display' style='font-size:1.25rem; color:#15803d; font-weight:700;'>Connected to Live NSE/BSE Streaming Pipes...</div>
    </div>
""", unsafe_allow_html=True)
...
# ==========================================
st.markdown("<div class='blueprint-container'><div style='font-size: 0.75rem; font-weight: 800; color: #0369a1; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 2px solid #0284c7; padding-bottom: 4px; margin-bottom: 6px;'>🔍 MANUAL CHECK OVERRIDE FIELD</div>", unsafe_allow_html=True)
