...
if evaluations:
    ...
    nav10a, nav10b = st.columns(2)
    if nav10a.button("❬ Previous", key="top10_prev", use_container_width=True, disabled=len(top10) < 2):
        st.session_state.top10_pointer = (idx10 - 1) % len(top10)
        st.rerun()
    if nav10b.button("Next ❭", key="top10_next", use_container_width=True, disabled=len(top10) < 2):
        st.session_state.top10_pointer = (idx10 + 1) % len(top10)
        st.rerun()

    score_color10 = "#15803d" if current10["technical_score"] >= MIN_WINNING_SCORE else "#64748b"
    st.markdown(
        '<div class="qb-panel" style="text-align:center;">'
        '<b style="font-size:1.2em;">#' + str(idx10 + 1) + ' — ' + current10["symbol"] + '</b><br>'
        '<span style="color:#0f172a;font-size:1.1em;">Rs ' + format(current10["ltp"], ",.2f") + '</span><br>'
        '<span style="color:' + score_color10 + ';font-weight:700;">' + str(current10["technical_score"]) + '/5</span> '
        '&nbsp;|&nbsp; Volume: ' + format(current10["volume"], ",") +
    ...
    # ---- Backtest-Validated Picks: among today's Top 10 qualifiers, find
    # up to 3 that ALSO have a history of working out, browsable ----
    qualifiers = [e for e in top10 if e["technical_score"] >= MIN_WINNING_SCORE]
    if qualifiers:
        with st.spinner(f"Cross-checking {len(qualifiers)} qualifying stocks against 7-day historical performance..."):
            bt_candidates = []
            for e in qualifiers:
                result = run_symbol_backtest(e["symbol"])
                if result["verdict"] == "PASS":
                    bt_candidates.append((e, result))
            bt_candidates.sort(key=lambda pair: pair[1]["win_rate"], reverse=True)
            bt_candidates = bt_candidates[:3]

        st.markdown("#### 🎯 Backtest-Validated Picks")
        if bt_candidates:
            if "bt_pointer" not in st.session_state:
                st.session_state.bt_pointer = 0
            st.session_state.bt_pointer %= len(bt_candidates)
            bidx = st.session_state.bt_pointer
            best_e, best_r = bt_candidates[bidx]

            st.caption(f"Showing {bidx + 1} of {len(bt_candidates)} backtest-validated picks (ranked by historical win rate)")
            navb1, navb2 = st.columns(2)
            if navb1.button("❬ Previous", key="bt_prev", use_container_width=True, disabled=len(bt_candidates) < 2):
                st.session_state.bt_pointer = (bidx - 1) % len(bt_candidates)
                st.rerun()
            if navb2.button("Next ❭", key="bt_next", use_container_width=True, disabled=len(bt_candidates) < 2):
                st.session_state.bt_pointer = (bidx + 1) % len(bt_candidates)
                st.rerun()

            bt_html = (
                '<div style="width:100%; background: linear-gradient(135deg, #ccfbf1 0%, #99f6e4 100%); '
                'border: 2px solid #0d9488; border-radius: 16px; padding: 24px; text-align:center; '
                'box-shadow: 0 4px 12px rgba(13,148,136,0.18);">'
                '<span style="background:#0d9488;color:white;padding:4px 14px;border-radius:16px;font-size:0.85em;">'
                '✅ LIVE SIGNAL + HISTORICAL BACKTEST PASS</span>'
                '<h1 style="margin:10px 0 0 0;">' + best_e["symbol"] + '</h1>'
                '<h2 style="color:#0d9488;margin:4px 0;">Rs ' + format(best_e["ltp"], ",.2f") + '</h2>'
                '<p>Live Technical Score: ' + str(best_e["technical_score"]) + '/5 &nbsp;|&nbsp; '
                'Historical Win Rate: ' + str(best_r["win_rate"]) + '% over ' + str(best_r["sample_count"]) +
                ' past signals &nbsp;|&nbsp; Avg forward move: ' + format(best_r["avg_return"], "+.2f") + '%</p>'
                '</div>'
            )
            st.markdown(bt_html, unsafe_allow_html=True)
            if st.button("View full details for " + best_e["symbol"], key="view_bt_winner"):
                st.session_state.manual_symbol = best_e["symbol"]
                st.rerun()
        else:
            st.info("None of today's Top 10 qualifying stocks have passed backtest validation yet (either the historical win rate was 50% or below, or there wasn't enough 7-day history to judge). The live 4/5+ signal above still stands on its own -- this second check is an extra filter, not a requirement.")
    else:
        st.caption("No stock in today's Top 5 currently meets the 4/5 live threshold, so backtest cross-checking is skipped for now.")
