...
if evaluations:
    ...
    for rank, e in enumerate(top10, start=1):
        ...
        max_score = len(e["rows"]) if segment != "NSE Equities" else 5
    ...
    if st.session_state.bt_hunt_result:
        ...
        cand_max_score = len(cand["rows"]) if segment != "NSE Equities" else 5
        if is_fallback:
...
# ---------------------------------------------------------- stock header --
exchange_label = "NSE" if segment == "NSE Equities" else ("COMEX/NYMEX (Proxy)" if segment == "Commodities (Global Proxy)" else "CRYPTO")
