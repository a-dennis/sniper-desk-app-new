...
if evaluations:
    ...
    # ---- Backtest-Validated Pick: one click hunts through the Top 10 in
    # priority order (live 4/5+ qualifiers first, since a live signal +
    # historical confirmation together is the strongest case; weaker-score
    # stocks only as a fallback), stopping at the first historical PASS it
    # finds. top10 is already sorted by score/volume, so no reordering
    # needed -- checking in that existing order already puts qualifiers
    # first automatically.
    ...
    if st.button(hunt_label):
        checked_this_hunt = []
        found = None
        with st.spinner(f"Hunting through the Top {len(top10)} for a historically validated stock..."):
            for e in top10:
    ...
    if st.session_state.bt_hunt_checked:
        st.caption(f"Last hunt checked {len(st.session_state.bt_hunt_checked)} of {len(top10)} stocks in the Top 10 before stopping.")
    ...
    if st.session_state.bt_hunt_result:
        ...
        if is_fallback:
            st.warning(f"⚠️ No stock with an active {MIN_WINNING_SCORE}/5+ live signal passed backtest this hunt -- "
                       f"falling back to {cand['symbol']}, which has a weaker live score ({cand['technical_score']}/5) "
        ...
        st.markdown(compact_card(
            ...
            "LIVE SCORE", str(cand["technical_score"]) + "/5", "#b45309" if is_fallback else "#15803d",
        ...
        st.info(f"No stock in the current Top {len(top10)} passed backtest validation this hunt. "
