...
ai_insight.py
--------------
An AI reasoning layer using Google's Gemini API (free tier: Flash-Lite,
~15 requests/minute, ~1,000+/day, no credit card needed).
...
# Tried in order -- Google renames/retires free-tier model aliases
# periodically, so we fall through to whichever is actually available for
# this API key rather than hardcoding one name that might not exist yet.
CANDIDATE_MODELS = ["gemini-2.5-flash-lite", "gemini-2.0-flash", "gemini-1.5-flash"]
...
def get_ai_insight(symbol: str, ltp: float, change_pct: float, technical_score: int,
    ...
    if genai is None:
        return {"success": False, "error": "google-generativeai package not installed.", "sections": {}}

    api_key = st.secrets.get("GEMINI_API_KEY")
    if not api_key:
        return {"success": False, "error": "GEMINI_API_KEY not found in Streamlit secrets.", "sections": {}}
    ...
    for model_name in CANDIDATE_MODELS:
        try:
            ...
            return {"success": True, "text": text, "error": None, "sections": sections, "model_used": model_name}
        except Exception as e:
            last_error = str(e)
            continue

    return {"success": False, "error": f"All model attempts failed. Last error: {last_error}", "sections": {}}


def _call_gemini(prompt: str) -> dict:
    """Shared low-level call used by the market pulse and journal coach
    functions below -- same model fallback behavior as get_ai_insight."""
    if genai is None:
        return {"success": False, "error": "google-generativeai package not installed.", "text": None}
    api_key = st.secrets.get("GEMINI_API_KEY")
    if not api_key:
