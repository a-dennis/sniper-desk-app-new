"""
kotak_client.py
----------------
NOTE: despite the filename (kept so app.py doesn't need any changes), this
now pulls data from free public Yahoo Finance data via yfinance, NOT from
Kotak Neo. This avoids needing broker API keys, TOTP, MPIN, or the SEBI
static-IP whitelisting requirement entirely, since we're not connecting to
any broker's order/trading system — just reading public market prices.

Trade-off: Yahoo's free data can occasionally lag by a few minutes and is an
unofficial source, so treat prices as "close to real-time" rather than
tick-perfect. Good enough for a personal screening tool.

No credentials or Streamlit secrets are required for this file to work.
"""

from __future__ import annotations

import datetime as dt
import io
import time
from dataclasses import dataclass
from typing import Optional

import pandas as pd
import requests
import streamlit as st
import yfinance as yf

IST = dt.timezone(dt.timedelta(hours=5, minutes=30))

# A curated list of liquid, well-known NSE stocks. Kept short on purpose —
# polling too many symbols too often against Yahoo's free (unofficial, rate
# limited) endpoint will get the app throttled. Feel free to add/remove
# symbols here (NSE trading symbol, no ".NS" suffix — that's added
# automatically).
# Trimmed to ~20 names on purpose: the app now scores EVERY stock in this
# list each cycle (not just checking volume), so a smaller, genuinely liquid
# set means fresher data per stock and fewer Yahoo rate-limit hiccups.
NSE_LIQUID_UNIVERSE = [
    "RELIANCE", "TCS", "HDFCBANK", "INFY", "ICICIBANK", "SBIN", "BHARTIARTL",
    "ITC", "LT", "KOTAKBANK", "AXISBANK", "BAJFINANCE", "MARUTI", "SUNPHARMA",
    "TITAN", "TATAMOTORS", "TATASTEEL", "ADANIENT", "HCLTECH", "JSWSTEEL",
]


def market_is_open(now: Optional[dt.datetime] = None) -> bool:
    """NSE cash market hours: 9:15–15:30 IST, Monday–Friday."""
    now = now or dt.datetime.now(IST)
    if now.weekday() >= 5:  # Sat/Sun
        return False
    open_t = now.replace(hour=9, minute=15, second=0, microsecond=0)
    close_t = now.replace(hour=15, minute=30, second=0, microsecond=0)
    return open_t <= now <= close_t


@dataclass
class Instrument:
    trading_symbol: str  # display name (e.g. "RELIANCE", "GOLD", "BTC")
    yahoo_ticker: str = ""  # actual Yahoo Finance symbol used for data calls
    instrument_token: str = ""  # unused with yfinance, kept for compatibility
    exchange_segment: str = "nse_cm"

    def __post_init__(self):
        if not self.yahoo_ticker:
            self.yahoo_ticker = f"{self.trading_symbol}.NS"  # default: NSE equity


# Global commodity futures used as a DIRECTIONAL PROXY for MCX movement --
# these are COMEX/NYMEX prices in USD, not actual MCX (INR) contract prices.
# Good for spotting momentum/breakouts to inform when to enter a real MCX
# trade; not for precise position sizing against real MCX contract specs.
COMMODITY_PROXIES = [
    ("GOLD", "GC=F"), ("SILVER", "SI=F"), ("CRUDEOIL", "CL=F"),
    ("NATURALGAS", "NG=F"), ("COPPER", "HG=F"),
]

# Major cryptocurrencies, trade 24/7 -- real global prices (not a proxy).
CRYPTO_LIST = [
    ("BTC", "BTC-USD"), ("ETH", "ETH-USD"), ("SOL", "SOL-USD"),
    ("BNB", "BNB-USD"), ("XRP", "XRP-USD"), ("ADA", "ADA-USD"), ("DOGE", "DOGE-USD"),
]


@st.cache_data(ttl=60 * 60 * 12, show_spinner=False)
def get_commodities_universe() -> list[Instrument]:
    return [Instrument(trading_symbol=name, yahoo_ticker=ticker) for name, ticker in COMMODITY_PROXIES]


@st.cache_data(ttl=60 * 60 * 12, show_spinner=False)
def get_crypto_universe() -> list[Instrument]:
    return [Instrument(trading_symbol=name, yahoo_ticker=ticker) for name, ticker in CRYPTO_LIST]


# Broad list of well-established, historically liquid NSE F&O-eligible
# stocks spanning banks, IT, auto, pharma, metals, energy, FMCG, infra,
# and financials. NSE reviews and revises the official F&O eligibility
# list periodically (based on market-wide position limits, volume, etc.),
# so this should be treated as a strong practical starting list, NOT a
# live authoritative source -- cross-check against NSE's current official
# F&O list before relying on it for real trades. Lot sizes are
# deliberately NOT included here for the same reason (they change too,
# and a stale lot size could cause a real order-sizing mistake) -- always
# confirm the current lot size on your broker's F&O order screen.
FNO_ELIGIBLE_STOCKS = [
    "RELIANCE", "TCS", "HDFCBANK", "INFY", "ICICIBANK", "SBIN", "BHARTIARTL", "ITC", "LT",
    "KOTAKBANK", "AXISBANK", "BAJFINANCE", "MARUTI", "SUNPHARMA", "TITAN", "TATAMOTORS",
    "TATASTEEL", "ADANIENT", "HCLTECH", "JSWSTEEL", "WIPRO", "ULTRACEMCO", "ASIANPAINT",
    "NESTLEIND", "POWERGRID", "NTPC", "M&M", "TECHM", "BAJAJFINSV", "COALINDIA", "SAIL",
    "BEL", "MOTHERSON", "NATIONALUM", "INDUSINDBK", "ONGC", "GRASIM", "HINDALCO", "DRREDDY",
    "CIPLA", "ADANIPORTS", "APOLLOHOSP", "BAJAJ-AUTO", "BPCL", "BRITANNIA", "DIVISLAB",
    "EICHERMOT", "GAIL", "GODREJCP", "HDFCLIFE", "HEROMOTOCO", "HINDUNILVR", "ICICIPRULI",
    "IOC", "INDIGO", "JINDALSTEL", "LTIM", "LUPIN", "MARICO", "PIDILITIND", "PNB", "SBILIFE",
    "SHREECEM", "SIEMENS", "TATACONSUM", "TATAPOWER", "TORNTPHARM", "TRENT", "TVSMOTOR",
    "UPL", "VEDANTA", "VOLTAS", "DLF", "HAVELLS", "ICICIGI", "IDFCFIRSTB", "MUTHOOTFIN",
    "NAUKRI", "PAGEIND", "PERSISTENT", "PFC", "RECLTD", "SRF", "ABB", "AMBUJACEM",
    "AUROPHARMA", "BANDHANBNK", "BANKBARODA", "BERGEPAINT", "BIOCON", "BOSCHLTD", "CANBK",
    "CHOLAFIN", "COLPAL", "CONCOR", "COFORGE", "CUMMINSIND", "DABUR", "DIXON", "ESCORTS",
    "EXIDEIND", "FEDERALBNK", "GLENMARK", "GMRINFRA", "GODREJPROP", "HAL", "HINDPETRO",
    "IDEA", "IGL", "INDHOTEL", "IRCTC", "JUBLFOOD", "LICHSGFIN", "LODHA", "M&MFIN",
    "MANAPPURAM", "MFSL", "MRF", "NMDC", "OBEROIRLTY", "PETRONET", "PIIND", "POLYCAB",
    "SBICARD", "SHRIRAMFIN", "SUNTV", "SYNGENE", "TATACHEM", "TATAELXSI", "UBL", "UNIONBANK",
    "WHIRLPOOL", "ZEEL",
]


@st.cache_data(ttl=60 * 60 * 12, show_spinner=False)
def get_fno_universe() -> list[Instrument]:
    return [Instrument(trading_symbol=s) for s in FNO_ELIGIBLE_STOCKS]


@st.cache_data(ttl=5 * 60, show_spinner=False)
def get_usdinr_rate() -> float:
    """Live USD->INR rate, used to convert commodity/crypto USD prices into
    Rupees so position sizing against a Rupee capital amount is meaningful.
    Falls back to a reasonable fixed estimate if the live rate can't be
    fetched -- better than crashing, but you should treat that fallback as
    approximate only."""
    try:
        hist = yf.Ticker("INR=X").history(period="2d", interval="1d")
        hist = hist.dropna(subset=["Close"])
        if not hist.empty:
            return float(hist["Close"].iloc[-1])
    except Exception:
        pass
    return 87.0  # rough fallback estimate if the live rate is unavailable


@st.cache_data(ttl=60 * 60 * 12, show_spinner=False)
def get_nse_equity_universe() -> list[Instrument]:
    """Curated fallback list, used only if the full official NSE list
    (get_full_nse_universe below) can't be fetched for some reason."""
    return [Instrument(trading_symbol=s) for s in NSE_LIQUID_UNIVERSE]


@st.cache_data(ttl=60 * 60 * 12, show_spinner=False)
def get_full_nse_universe() -> list[Instrument]:
    """Pulls the complete official NSE cash-market equity list (~2,000
    symbols) directly from NSE's own published archive. This is what makes
    the scanner cover 'all NSE stocks' rather than a fixed short list.

    NSE moved this file from archive.nseindia.com to nsearchives.nseindia.com,
    and their server also expects a browser-like visit first (it sets
    cookies on the homepage that the CSV request then needs) -- a bare
    request without that handshake gets rejected. This does both steps in
    one requests.Session().

    Falls back to the curated liquid list if NSE's endpoint is unreachable
    or blocks the request anyway (their site does sometimes tighten
    anti-bot measures further).
    """
    url = "https://nsearchives.nseindia.com/content/equities/EQUITY_L.csv"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
                      "(KHTML, like Gecko) Chrome/124.0 Safari/537.36",
        "Accept": "text/csv,application/csv,*/*",
        "Accept-Language": "en-US,en;q=0.9",
        "Referer": "https://www.nseindia.com/market-data/securities-available-for-trading",
    }
    try:
        session = requests.Session()
        session.headers.update(headers)
        session.get("https://www.nseindia.com", timeout=10)  # handshake -- sets required cookies
        resp = session.get(url, timeout=15)
        resp.raise_for_status()
        df = pd.read_csv(io.BytesIO(resp.content))
        sym_col = next(c for c in df.columns if "SYMBOL" in c.upper())
        series_col = next((c for c in df.columns if "SERIES" in c.upper()), None)
        if series_col:
            df = df[df[series_col].astype(str).str.strip().str.upper() == "EQ"]
        symbols = df[sym_col].dropna().astype(str).str.strip().tolist()
        if len(symbols) < 500:  # sanity check -- something looked wrong, don't trust a tiny result
            raise ValueError("Unexpectedly small NSE universe returned")
        return [Instrument(trading_symbol=s) for s in symbols]
    except Exception:
        return get_nse_equity_universe()


@st.cache_data(ttl=15 * 60, show_spinner=False)
def get_tier1_shortlist(instruments: list[Instrument], top_n: int = 50, batch_size: int = 400) -> list[Instrument]:
    """Tier 1: cheap batch daily-quote scan to find the most actively traded
    stocks right now, refreshed every 15 minutes.

    Scanning all ~2,000 NSE stocks in one go is too slow for a live page
    (each Yahoo batch request takes real time, and 2,000 symbols means many
    batches back to back). Instead, each 15-minute cycle scans a rotating
    slice of ~400 stocks from the full universe -- fast enough to stay
    responsive, while rotating through a different slice each cycle so the
    full ~2,000-stock universe gets covered over roughly the course of an
    hour rather than never at all.
    """
    total = len(instruments)
    if total > batch_size:
        num_slices = (total // batch_size) + 1
        slot = int(time.time() // (15 * 60)) % num_slices
        start = slot * batch_size
        pool = instruments[start:start + batch_size]
        if len(pool) < batch_size:  # wrap around near the end of the list
            pool = pool + instruments[:batch_size - len(pool)]
    else:
        pool = instruments

    symbols = [i.trading_symbol for i in pool]
    ranked = []
    chunk_size = 200
    for start in range(0, len(symbols), chunk_size):
        chunk = symbols[start:start + chunk_size]
        tickers = " ".join(f"{s}.NS" for s in chunk)
        try:
            data = yf.download(tickers, period="2d", interval="1d", group_by="ticker",
                                threads=True, progress=False)
        except Exception:
            continue
        for s in chunk:
            col = f"{s}.NS"
            try:
                sub = data[col] if len(chunk) > 1 else data
                if sub.empty or sub["Volume"].isna().all():
                    continue
                last_vol = float(sub["Volume"].iloc[-1])
                ranked.append((s, last_vol))
            except Exception:
                continue
    ranked.sort(key=lambda x: x[1], reverse=True)
    top_symbols = [s for s, _ in ranked[:top_n]]
    if not top_symbols:
        return get_nse_equity_universe()[:top_n]
    return [Instrument(trading_symbol=s) for s in top_symbols]


INDEX_TICKERS = {"NIFTY 50": "^NSEI", "BANK NIFTY": "^NSEBANK"}


@st.cache_data(ttl=30, show_spinner=False)
def get_index_quotes() -> dict[str, dict]:
    """Live (a few minutes' lag) NIFTY 50 / BANK NIFTY quotes. Returns
    {"NIFTY 50": {"ltp":..., "change":..., "change_pct":...}, ...}."""
    out = {}
    for name, sym in INDEX_TICKERS.items():
        try:
            hist = yf.Ticker(sym).history(period="2d", interval="1d")
            if hist.empty:
                continue
            last = hist.iloc[-1]
            prev_close = hist["Close"].iloc[-2] if len(hist) > 1 else last["Close"]
            ltp = float(last["Close"])
            change = ltp - float(prev_close)
            change_pct = (change / prev_close * 100) if prev_close else 0.0
            out[name] = {"ltp": ltp, "change": change, "change_pct": change_pct}
        except Exception:
            continue
    return out


@st.cache_data(ttl=20, show_spinner=False)
def get_live_quotes(instruments: list[Instrument]) -> dict[str, dict]:
    """Batch 'live' (a few minutes' lag, free-tier) quotes for a list of
    instruments. Returns {trading_symbol: {"ltp", "volume", "change",
    "change_pct"}}. Cached 20s so we don't hammer Yahoo on every rerun."""
    out = {}
    for inst in instruments:
        try:
            hist = yf.Ticker(inst.yahoo_ticker).history(period="5d", interval="1d")
            hist = hist.dropna(subset=["Close"])  # Yahoo sometimes appends an incomplete/NaN row
            if hist.empty:
                continue
            last = hist.iloc[-1]
            prev_close = hist["Close"].iloc[-2] if len(hist) > 1 else last["Close"]
            ltp = float(last["Close"])
            change = ltp - float(prev_close)
            change_pct = (change / prev_close * 100) if prev_close else 0.0
            out[inst.trading_symbol] = {
                "ltp": ltp,
                "volume": int(last["Volume"]),
                "change": change,
                "change_pct": change_pct,
            }
        except Exception:
            continue  # skip symbols Yahoo has trouble with rather than crash the app
    return out


@st.cache_data(ttl=30, show_spinner=False)
def get_intraday_candles(yahoo_ticker: str, interval: str = "1m") -> pd.DataFrame:
    """1-minute OHLCV candles for the most recent trading session, for any
    Yahoo ticker (NSE equity, commodity future, or crypto pair).

    Tries finer granularity first, then falls back to coarser intervals --
    Yahoo often doesn't reliably offer 1-minute data for commodity futures
    the way it does for equities/crypto, so without this fallback the chart
    would just come back empty for those. Also tries a longer lookback
    period if the shorter one comes back empty (Yahoo's "1d" period is a
    calendar day, not a trading day, so it can return nothing at all right
    after a weekend or holiday).

    "Most recent date" is bucketed using IST explicitly (converting first if
    the index isn't already IST-labeled) -- without this, if Yahoo ever
    returns timestamps in a different timezone, the day-boundary could be
    computed incorrectly and silently drop early-morning bars, which would
    then make anything relying on the 9:15-9:30 opening range (like the
    Breakout Setup Detector) wrongly report "not enough data" all day.
    """
    for candidate_interval in (interval, "5m", "15m"):
        for period in ("5d", "10d"):
            try:
                df = yf.Ticker(yahoo_ticker).history(period=period, interval=candidate_interval)
            except Exception:
                continue
            if df.empty:
                continue
            df = df.rename(columns={c: c.lower() for c in df.columns})
            expected = ["open", "high", "low", "close", "volume"]
            for col in expected:
                if col not in df.columns:
                    df[col] = pd.NA
            df = df[expected].dropna(subset=["open", "high", "low", "close"])
            if df.empty:
                continue
            idx_ist = df.index.tz_localize(IST) if df.index.tz is None else df.index.tz_convert(IST)
            last_date = idx_ist.date.max()
            result = df[idx_ist.date == last_date]
            if not result.empty:
                return result
        if candidate_interval == interval and candidate_interval != "5m":
            continue  # move on to coarser interval fallback
    return pd.DataFrame()
