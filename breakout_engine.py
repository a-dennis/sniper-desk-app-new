...
def get_opening_range(candles: pd.DataFrame):
    """Returns (or_high, or_low) from the opening portion of the most recent
    trading day present in `candles`.

    Uses the first 3 candles of the day POSITIONALLY (not by matching an
    exact "9:15-9:30" clock window) -- Yahoo's free intraday bar timestamps
    aren't always aligned exactly to the literal market-open minute, and a
    strict clock-time match would silently fail (returning no opening range
    at all, for every stock, all day) if there's even a few minutes of
    misalignment. On 5-minute candles, the first 3 bars cover the same
    ~15 minutes the 9:15-9:30 window was meant to represent, without being
    fragile to exact timestamp alignment.
    """
    if candles.empty:
        return None, None
    idx = candles.index
    ...
    day_candles = candles.loc[idx.date == last_date]
    if day_candles.empty:
        return None, None
    window = day_candles.iloc[:3]
    if window.empty:
        return None, None
...
def detect_setup(candles: pd.DataFrame) -> dict:
    ...
    if or_high is None:
        result["detail"] = "Opening range not found in the available candle history."
        return result

    # Everything after the first 3 (opening-range) candles -- positional,
    # for the same reason get_opening_range uses position instead of exact
    # clock-time matching.
    post_or = candles.iloc[3:].copy()
    post_or["atr"] = atr.iloc[3:]
